...
def title() -> str:
  return "Evening Star (CDLEVENINGSTAR) - 黄昏星"
...
def calc(history_data: pd.DataFrame, options: dict = defaultOptions) -> pd.DataFrame:
  ...
  Returns:
      ...
      - 'CDLEVENINGSTAR': 100 (看涨), -100 (看跌), 0 (未识别)
      - 'Signal': 信号 (1: 看涨, -1: 看跌, 0: 中性)
  """
  options = defaultOptions | options
  cdleveningstar = talib.CDLEVENINGSTAR(history_data[options['column_open']],
  ...
  result['Signal'] = 0
  result.loc[cdleveningstar == 100, 'Signal'] = 1  # Bullish pattern
  result.loc[cdleveningstar == -100, 'Signal'] = -1 # Bearish pattern
