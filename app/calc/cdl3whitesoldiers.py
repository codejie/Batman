...
def title() -> str:
  return "Three White Soldiers (CDL3WHITESOLDIERS) - 三白兵"
...
def calc(history_data: pd.DataFrame, options: dict = defaultOptions) -> pd.DataFrame:
  ...
  Returns:
      ...
      - 'CDL3WHITESOLDIERS': 100 (看涨), -100 (看跌), 0 (未识别)
      - 'Signal': 信号 (1: 看涨, -1: 看跌, 0: 中性)
  """
  options = defaultOptions | options
  cdl3whitesoldiers = talib.CDL3WHITESOLDIERS(history_data[options['column_open']],
  ...
  result['Signal'] = 0
  result.loc[cdl3whitesoldiers == 100, 'Signal'] = 1  # Bullish pattern
  result.loc[cdl3whitesoldiers == -100, 'Signal'] = -1 # Bearish pattern
