...
def calc(history_data: pd.DataFrame, options: dict = defaultOptions) -> pd.DataFrame:
  ...
  Returns:
      ...
      - 'CDLDOJI': 100 (看涨), -100 (看跌), 0 (未识别)
      - 'Signal': 信号 (1: 看涨, -1: 看跌, 0: 中性)
  """
  options = defaultOptions | options
  cdldoji = talib.CDLDOJI(history_data[options['column_open']],
  ...
  result['Signal'] = 0
  result.loc[cdldoji == 100, 'Signal'] = 1  # Bullish pattern
  result.loc[cdldoji == -100, 'Signal'] = -1 # Bearish pattern
